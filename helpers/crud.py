...
def add_employees(db: Session, employee_list):
    for item in employee_list['employees']['employee']:
        ...
        role_id = item['mainRoleId'] if 'mainRoleId' in item else None
        role_ids = [item['rolesIds']] if 'rolesIds' in item else None
        role_codes = [item['roleCodes']] if 'roleCodes' in item else None
        role_code = item['mainRoleCode'] if 'mainRoleCode' in item else None
        deleted = item['deleted'] if 'deleted' in item else None
        deleted = False if deleted == 'false' else True
        supplier = item['supplier'] if 'supplier' in item else None
        supplier = False if supplier == 'false' else True
        employee = item['employee'] if 'employee' in item else None
        employee = False if employee == 'false' else True
        client = item['client'] if 'client' in item else None
        client = False if client == 'false' else True
        representstore = item['representsStore'] if 'representsStore' in item else None
        representstore = False if representstore == 'false' else True
